(function(window) {
    ...
    $fileInput.find(':file').change(function() {
		...
		var oldauditquery = new Parse.Query(Parse.Object.extend('Audit'));
		var oldaudit = oldauditquery.get(audit.id);
		var oldsales22 = oldaudit.get('sales');
		Parse.Object.destroyAll(oldsales22);

		
		oldsalesQuery.find().then(function (oldsales) {
			for (var j = 0; j < oldsales.length; j++) {
				var mysale=oldsales[j];
				mysale.destroy();
			}
		...
		var oldsalesQuery = new Parse.Query(Parse.Object.extend('Sale'));
		oldsalesQuery.equalTo('audit', audit);
		oldsalesQuery.limit('1000');
		...
		var oldsalesQuery1 = new Parse.Query(Parse.Object.extend('Sale'));
		oldsalesQuery1.equalTo('audit', audit.get('objectId'));
		
		oldsalesQuery1.find().then(function (oldsales1) {
			Parse.Object.destroyAll(oldsales1);
		});

		var oldsalesQuery2 = new Parse.Query(Parse.Object.extend('Sale'));
		var myAudit = new Audit();
		myAudit.id=audit.id;
		
		oldsalesQuery2.equalTo('audit', myAudit);
		
		oldsalesQuery2.find().then(function (oldsales2) {
			Parse.Object.destroyAll(oldsales1);
		});
...
//						var oldsalesQuery = new Parse.Query(Sale);
//						var oldsaleIds = audit.get('sales').map(function(sale1) { return sale1.id; });
//						oldsalesQuery.containedIn('objectId', oldsaleIds);
